class Map {
	...
	generate(){
        ...
        for (let n=0; n<canvas.width/tileSize; n++){
        ...
        let mapSquares = [];
            ...
            mapSquares.forEach((v2, i2, a2) => { 
                ...
                let rndm = _.random(40 - v2[2]/100, 120 - v2[2]/100);
                ...
                for (let x = 0; x < v2[2]; x += tileSize) {
                    for (let y = 0; y < v2[3]; y += tileSize) {
                        ...
                        ctx.fillRect(Math.floor(x+v2[0]), Math.floor(y+v2[1]), tileSize, tileSize)
                        tiles[Math.floor((x+v2[0])/4)][Math.floor((y+v2[1])/4)] = {info: tileType.getBiomeInfo(), pos:{x:Math.floor((x+v2[0])/4), y:Math.floor((y+v2[1])/4)}}
                    }
            ...
            tiles.forEach((subArr, index) => { // fill 0 of tiles with the sea biome
                subArr.forEach((v, i, a) => {
                    if(v === 0) {
                        let biome = new Sea();
                        a[i] = {info: biome.getBiomeInfo(), pos:{x:index, y:i}}
                    }
                })
            })
